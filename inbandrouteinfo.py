...
def _inrouteinfo_(): #Captura informacion de las rutas de las llamadas entrantes.
    ...
    url = "https://" + data.host + ":8088/api/v1.1.0/inroute/query?&token=" + token
    payload="{ \"name\":\"all\"}"
    # payload="{ \"name\":\"" + name + "\"}"
    headers ={
        'host': data.host,
    ...
    with open(data.ruta_json_inrouteinfo, 'w') as json_file:
